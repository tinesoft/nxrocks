...
export function addGradlePlugin(tree: Tree, rootFolder: string, language: 'java' | 'kotlin' | 'groovy', pluginId: string, pluginVersion: string) {
...
export function applySpotlessGradlePlugin(tree: Tree, rootFolder:string, language: 'java' | 'kotlin' | 'groovy', jdkVersion?: number, gitBaseBranch?: string):boolean {
    const ext = language === 'kotlin' ? '.gradle.kts' : '.gradle';
    const buildGradle = tree.read(`${rootFolder}/build${ext}`, 'utf-8');
