...
export function addMavenPublishPlugin(tree: Tree, options: NormalizedSchema) {
    if (options.buildSystem === 'GRADLE' || options.buildSystem === 'GRADLE_KOTLIN_DSL') {
        logger.debug(`Adding 'maven-publish' plugin...`);

        addGradlePlugin(tree, options.projectRoot, 'java', 'maven-publish', undefined, options.buildSystem === 'GRADLE_KOTLIN_DSL');

        const artifactSource = 'jar';
        const publishing = options.buildSystem === 'GRADLE_KOTLIN_DSL' ?
        stripIndent`
        publishing {
        	publications {
        ...
        const disableEnforcedPlatformValidation = options.buildSystem === 'GRADLE_KOTLIN_DSL' ?
        stripIndent`
