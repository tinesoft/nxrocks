...
async function main() {
  ...
  const options = (yargs
    ...
    .command(
      '$0 [name]',
      'Create a new Nx workspace with Spring Boot support',
  ...
  useNxCloud ??= await confirm({
    message: 'Would you like to use Nx Cloud? [ https://nx.app/?utm_source=create-nx-spring-boot ]',
    initialValue: false
  }) as boolean;
  ...
  const notes = [
    '- Go to [ https://github.com/tinesoft/nxrocks/tree/develop/packages/nx-spring-boot ] to get started with Nx and Spring Boot plugin.',
    `- Run '${getNxCommand(useNxWrapper)} g @nxrocks/nx-spring-boot:project' to later add additional projects into this workspace.`
  ];
  ...
  if (useNxWrapper) {
    ...
    directory = createWorkspaceWithNxWrapper(name, '@nxrocks/nx-spring-boot', allArgs, useNxCloud, presetVersion, !verbose);

    s.stop(`Successfully created the workspace: ${name}`);
  }
  else {
    directory = (await createWorkspace(
      `@nxrocks/nx-spring-boot@${presetVersion}`,
      {
        name: name,
        nxCloud: useNxCloud,
        packageManager: 'npm',
