...
export function getPackageInfo(project: ProjectConfiguration): PackageInfo {
  if (fileExists(getProjectFilePath(project, 'pubspec.yaml'))) {
    ...
    const pubspec = load(
      getProjectFileContent(project, 'pubspec.yaml')
    ) as Pubspec;

    const dependencies: PackageInfo[] = [];

    Object.keys(
      Object.assign({}, pubspec.dependencies, pubspec.dev_dependencies)
    ).forEach((depId) => {
  ...
  throw new Error(
    `Cannot inspect dependencies of Flutter projet at: '${getProjectRoot(
      project
