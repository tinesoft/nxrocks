...
import { normalizeOptions, generateKtorProject, addMavenPublishPlugin, addFormattingWithSpotless, promptKtorFeatures, addDockerfile } from './lib';
import { addPluginToNxJson, BuilderCommandAliasType, NX_KTOR_PKG,  } from '@nxrocks/common';
...
export async function projectGenerator(tree: Tree, options: ProjectGeneratorOptions) {
  ...
  for (const command of commands) {
    targets[command] = {
      ...
      ...( ['publish-image', 'publish-image-locally', 'run-docker'].includes(command) ? { dependsOn: ['build-image'] } :{}),
      ...( ['build', 'build-image', 'install', 'test'].includes(command) ? {outputs: [`{workspaceRoot}/${normalizedOptions.projectRoot}/${normalizedOptions.buildSystem === 'MAVEN' ? 'target' : 'build'}`]}: {})
    };
  }
