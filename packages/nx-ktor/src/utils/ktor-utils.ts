...
import { isMavenProject, checkProjectBuildFileContains, isGradleProject, BuilderCommandAliasType, hasGradleProject, hasMavenProject, runBuilderCommand, getCommonHttpHeaders, NX_KTOR_PKG } from '@nxrocks/common';
...
export const DEFAULT_KTOR_INITIALIZR_URL = 'https://ktor-plugin.europe-north1-gke.intellij.net';//'https://start.ktor.io';
...
export function runKtorPluginCommand(
    ...
    console.log(`runKtorPluginCommand, ignoreWrapper: ${options.ignoreWrapper}, useLegacyWrapper: ${options.useLegacyWrapper}`);
...
export function isKtorProject(project: ProjectConfiguration): boolean {
    ...
    if (isMavenProject(project)) {
        return checkProjectBuildFileContains(project, { fragments: ['<version>${ktor_version}</version>'] });
    }

    if (isGradleProject(project)) {
        return checkProjectBuildFileContains(project, { fragments: ['id \'io.ktor.plugin\'', 'id("io.ktor.plugin")'] });
    }

    return false;
}

export async function fetchKtorFeatures(options: NormalizedSchema): Promise<KtorFeature[]> {
    const version = options.ktorVersion ?? 'main';
    const url = `https://raw.githubusercontent.com/ktorio/ktor/${version}/settings.gradle.kts`;
    const response = await fetch(url, getCommonHttpHeaders(NX_KTOR_PKG, url, options.proxyUrl));

    const pluginLine = 'include(":ktor-server:ktor-server-plugins:ktor-server-';
    return (await response.text())?.split(/\r\n|\r|\n/)
        .filter(line => line.trim().startsWith(pluginLine))
        .map(plugin => ({ name: plugin.replace(pluginLine, '').replace('")', '') })) ?? [];
