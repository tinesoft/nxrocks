...
async function main() {
  ...
  const options = (yargs
    ...
    .command(
      '$0 [name]',
      'Create a new Nx workspace with Ktor support',
  ...
  useNxCloud ??= await confirm({
    message: 'Would you like to use Nx Cloud? [ https://nx.app/?utm_source=create-nx-ktor ]',
    initialValue: false
  }) as boolean;
  ...
  const notes = [
    '- Go to [ https://github.com/tinesoft/nxrocks/tree/develop/packages/nx-ktor ] to get started with Nx and Ktor plugin.',
    `- Run '${getNxCommand(useNxWrapper)} g @nxrocks/nx-ktor:project' to later add additional projects into this workspace.`
  ];
  ...
  if (useNxWrapper) {
    ...
    directory = createWorkspaceWithNxWrapper(name, '@nxrocks/nx-ktor', allArgs, useNxCloud, presetVersion, !verbose);

    s.stop(`Successfully created the workspace: ${name}`);
  }
  else {
    directory = (await createWorkspace(
      `@nxrocks/nx-ktor@${presetVersion}`,
      {
        name: name,
        nxCloud: useNxCloud,
        packageManager: 'npm',
