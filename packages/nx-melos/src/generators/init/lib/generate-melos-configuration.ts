import { Tree, logger, generateFiles, getWorkspaceLayout } from '@nrwl/devkit';
import { readPackageJson } from 'nx/src/project-graph/file-utils';
import { getDartSDKVersion } from '../../../utils/melos-utils';
import { join } from 'path';

export async function generateMelosConfigurationFile(tree: Tree): Promise<void> {
    ...
    const usePubspecOverrides = +(dartVersions?.[0]) >= 2 && +(dartVersions?.[1]) >=17 && +(dartVersions?.[2]) >= 0;
