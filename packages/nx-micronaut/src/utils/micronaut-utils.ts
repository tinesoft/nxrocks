...
export async function fetchMicronautFeatures(options:  NormalizedSchema): Promise<MicronautFeature[]> {
    const response = await fetch(`${options.micronautLaunchUrl}/application-types/${options.projectType}/features`, getCommonHttpHeaders(NX_MICRONAUT_PKG, options.proxyUrl, options.micronautLaunchUrl));
